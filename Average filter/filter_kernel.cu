  ...
  // if((threadIdx.x >= FILTER_RADIUS) && (threadIdx.x <( BLOCK_WIDTH - FILTER_RADIUS)) &&(threadIdx.y >= FILTER_RADIUS) && (threadIdx.y < (BLOCK_HEIGHT - FILTER_RADIUS)))
   if(x<width && y<height)
   {
	...
	for(int dy = -FILTER_RADIUS; dy <= FILTER_RADIUS; ++dy)
	   ...
	   {
        	//float pixelValue = (float)(sharedMem[(dy+FILTER_RADIUS) * blockDim.x +( dx+FILTER_RADIUS)]);
               // float index1 = (float)((dy+FILTER_RADIUS) +( dx+FILTER_RADIUS));
		...
		//sum += pixelValue;
               ...
               // sumX+= sharedMem[dy +FILTER_RADIUS];
                //sumY+= sharedMem[dx +FILTER_RADIUS];
                //float pixel = width * (y+dx)+(x+dx);
              //  sum + = (float)(sharedMem[shareIndex+ (FILTER_RADIUS +dx)+(FILTER_RADIUS+dx)]);
                //sum = sum + shredMem[index];
                //flag++;
                int index1 = (dx+y) * (width) + (x+dy);
                sum = sum +g_DataIn[index1];
           }
        }
        
        //sum = sumX+sumY;

        g_DataOut[index] = static_cast<unsigned char>(sum / FILTER_AREA);
         // g_DataOut[index] = (sum/flag) > EDGE_VALUE_THRESHOLD ? 255 : 0;//1023 : 0
